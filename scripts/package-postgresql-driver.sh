...
source_framework="$products/PackageFrameworks/QueryCraftPostgreSQLDriver.framework"
...
bundle="$output_root/PostgreSQL-$driver_arch.querycraftdriver"
macos="$bundle/Contents/MacOS"
frameworks="$bundle/Contents/Frameworks"
info_plist="$bundle/Contents/Info.plist"
manifest="$output_root/postgresql-$driver_arch.json"
...
cleanup() {
    ...
    rm -rf "$output_root/PostgreSQL-$driver_arch.querycraftdriver.previous"
}
trap cleanup EXIT

source_binary="$source_framework/Versions/A/QueryCraftPostgreSQLDriver"
...
mkdir -p "$output_root"
if [[ -e "$bundle" ]]; then
    mv "$bundle" "$output_root/PostgreSQL-$driver_arch.querycraftdriver.previous"
fi
mkdir -p "$macos" "$frameworks"
binary="$macos/QueryCraftPostgreSQLDriver"
...
copy_framework_dependencies() {
    ...
    otool -L "$input_binary" | awk '/@rpath\/.*\.framework\// { print $1 }' | while read -r dependency; do
        ...
        [[ "$framework_name" != "QueryCraftPostgreSQLDriver.framework" ]] || continue
...
thin_binary() {
    ...
    thin_temp="$(mktemp "$output_root/.postgresql-thin-$driver_arch.XXXXXX")"
...
/usr/libexec/PlistBuddy -c 'Add :CFBundleExecutable string QueryCraftPostgreSQLDriver' "$info_plist"
/usr/libexec/PlistBuddy -c 'Add :CFBundleIdentifier string io.github.future0923.QueryCraft.Driver.PostgreSQL' "$info_plist"
/usr/libexec/PlistBuddy -c 'Add :CFBundleInfoDictionaryVersion string 6.0' "$info_plist"
/usr/libexec/PlistBuddy -c 'Add :CFBundleName string PostgreSQL Driver' "$info_plist"
...
/usr/libexec/PlistBuddy -c 'Add :NSPrincipalClass string QueryCraftPostgreSQLDriverEntry' "$info_plist"
/usr/libexec/PlistBuddy -c 'Add :QCDriverDatabaseType string postgresql' "$info_plist"
...
archive_temp="$(mktemp "$output_root/.postgresql-driver-$driver_version-$driver_arch.XXXXXX")"
ditto -c -k --keepParent "$bundle" "$archive_temp"
sha256="$(shasum -a 256 "$archive_temp" | cut -d ' ' -f 1)"
size="$(stat -f %z "$archive_temp")"
archive_name="postgresql-driver-$driver_version-$driver_arch-${sha256[1,16]}.zip"
...
manifest_temp="$(mktemp "$output_root/.postgresql-$driver_arch.json.XXXXXX")"
sed \
    ...
    "$project_root/scripts/postgresql-driver-manifest.json.in" > "$manifest_temp"
querycraft_validate_driver_package_metadata \
    "$info_plist" "$manifest_temp" postgresql \
...
for stale_archive in "$output_root"/postgresql-driver-*.zip(N); do
    keep_archive=0
    for current_manifest in \
        "$output_root/postgresql-x86_64.json" \
        "$output_root/postgresql-arm64.json"
    do
...
rm -f "$output_root/postgresql.json"
rm -rf "$output_root/PostgreSQL.querycraftdriver"
for stale_bundle in "$output_root"/PostgreSQL*.querycraftdriver\ *(N); do
    rm -rf "$stale_bundle"
done
