...
source_framework="$products/PackageFrameworks/QueryCraftRedisDriver.framework"
...
bundle="$output_root/Redis-$driver_arch.querycraftdriver"
macos="$bundle/Contents/MacOS"
info_plist="$bundle/Contents/Info.plist"
manifest="$output_root/redis-$driver_arch.json"
...
cleanup() {
    ...
    rm -rf "$output_root/Redis-$driver_arch.querycraftdriver.previous"
}
trap cleanup EXIT

source_binary="$source_framework/Versions/A/QueryCraftRedisDriver"
...
mkdir -p "$output_root"
if [[ -e "$bundle" ]]; then
    mv "$bundle" "$output_root/Redis-$driver_arch.querycraftdriver.previous"
fi
mkdir -p "$macos"
binary="$macos/QueryCraftRedisDriver"
...
if [[ "$target_architectures" != "$driver_arch" ]]; then
    ...
    thin_temp="$(mktemp "$output_root/.redis-thin-$driver_arch.XXXXXX")"
...
/usr/libexec/PlistBuddy -c 'Add :CFBundleExecutable string QueryCraftRedisDriver' "$info_plist"
/usr/libexec/PlistBuddy -c 'Add :CFBundleIdentifier string io.github.future0923.QueryCraft.Driver.Redis' "$info_plist"
/usr/libexec/PlistBuddy -c 'Add :CFBundleInfoDictionaryVersion string 6.0' "$info_plist"
/usr/libexec/PlistBuddy -c 'Add :CFBundleName string Redis Driver' "$info_plist"
...
/usr/libexec/PlistBuddy -c 'Add :NSPrincipalClass string QueryCraftRedisDriverEntry' "$info_plist"
/usr/libexec/PlistBuddy -c 'Add :QCDriverDatabaseType string redis' "$info_plist"
...
archive_temp="$(mktemp "$output_root/.redis-driver-$driver_version-$driver_arch.XXXXXX")"
ditto -c -k --keepParent "$bundle" "$archive_temp"
sha256="$(shasum -a 256 "$archive_temp" | cut -d ' ' -f 1)"
size="$(stat -f %z "$archive_temp")"
archive_name="redis-driver-$driver_version-$driver_arch-${sha256[1,16]}.zip"
...
manifest_temp="$(mktemp "$output_root/.redis-$driver_arch.json.XXXXXX")"
sed \
    ...
    "$project_root/scripts/redis-driver-manifest.json.in" > "$manifest_temp"
querycraft_validate_driver_package_metadata \
    "$info_plist" "$manifest_temp" redis \
    "$driver_version" "$driver_build" "$driver_arch"
mv "$manifest_temp" "$manifest"
manifest_temp=""

for stale_archive in "$output_root"/redis-driver-*.zip(N); do
    keep_archive=0
    for current_manifest in \
        "$output_root/redis-x86_64.json" \
        "$output_root/redis-arm64.json"
    do
...
rm -f "$output_root/redis.json"
rm -rf "$output_root/Redis.querycraftdriver"
for stale_bundle in "$output_root"/Redis*.querycraftdriver\ *(N); do
    rm -rf "$stale_bundle"
done
