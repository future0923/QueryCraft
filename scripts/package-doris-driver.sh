...
source_framework="$products/PackageFrameworks/QueryCraftDorisDriver.framework"
...
bundle="$output_root/Doris-$driver_arch.querycraftdriver"
macos="$bundle/Contents/MacOS"
frameworks="$bundle/Contents/Frameworks"
info_plist="$bundle/Contents/Info.plist"
manifest="$output_root/doris-$driver_arch.json"
...
cleanup() {
    ...
    rm -rf "$output_root/Doris-$driver_arch.querycraftdriver.previous"
}
trap cleanup EXIT

source_binary="$source_framework/Versions/A/QueryCraftDorisDriver"
...
mkdir -p "$output_root"
if [[ -e "$bundle" ]]; then
    mv "$bundle" "$output_root/Doris-$driver_arch.querycraftdriver.previous"
fi
mkdir -p "$macos" "$frameworks"
binary="$macos/QueryCraftDorisDriver"
...
copy_framework_dependencies() {
    ...
    otool -L "$input_binary" | awk '/@rpath\/.*\.framework\// { print $1 }' | while read -r dependency; do
        ...
        [[ "$framework_name" != "QueryCraftDorisDriver.framework" ]] || continue
...
thin_binary() {
    ...
    thin_temp="$(mktemp "$output_root/.doris-thin-$driver_arch.XXXXXX")"
...
/usr/libexec/PlistBuddy -c 'Add :CFBundleExecutable string QueryCraftDorisDriver' "$info_plist"
/usr/libexec/PlistBuddy -c 'Add :CFBundleIdentifier string io.github.future0923.QueryCraft.Driver.Doris' "$info_plist"
/usr/libexec/PlistBuddy -c 'Add :CFBundleInfoDictionaryVersion string 6.0' "$info_plist"
/usr/libexec/PlistBuddy -c 'Add :CFBundleName string Doris Driver' "$info_plist"
...
archive_temp="$(mktemp "$output_root/.doris-driver-$driver_version-$driver_arch.XXXXXX")"
ditto -c -k --keepParent "$bundle" "$archive_temp"
sha256="$(shasum -a 256 "$archive_temp" | cut -d ' ' -f 1)"
size="$(stat -f %z "$archive_temp")"
archive_name="doris-driver-$driver_version-$driver_arch-${sha256[1,16]}.zip"
...
manifest_temp="$(mktemp "$output_root/.doris-$driver_arch.json.XXXXXX")"
sed \
...
for stale_archive in "$output_root"/doris-driver-*.zip(N); do
    keep_archive=0
    for current_manifest in \
        "$output_root/doris-x86_64.json" \
        "$output_root/doris-arm64.json"
    do
...
rm -f "$output_root/doris.json"
rm -rf "$output_root/Doris.querycraftdriver"
for stale_bundle in "$output_root"/Doris*.querycraftdriver\ *(N); do
    rm -rf "$stale_bundle"
done
