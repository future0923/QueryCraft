...
extension TextView {
    ...
    public func replaceCharacters(
        ...
        // An outer batch (such as undo) has not updated the line storage until its final endEditing().
        if !skipUpdateSelection && textStorage.editedMask.isEmpty {
        ...
        // Check the current insertion offset, not the previous frame of the native insertion indicator.
        if !skipUpdateSelection, let rect = selectionScrollRect(), !visibleRect.contains(rect) {
