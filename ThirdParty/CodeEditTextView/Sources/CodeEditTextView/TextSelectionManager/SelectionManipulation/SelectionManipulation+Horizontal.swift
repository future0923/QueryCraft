...
package extension TextSelectionManager {
    ...
    private func extendSelectionVisualLine(string: NSString, from offset: Int, delta: Int) -> NSRange {
        guard let layoutManager,
              let line = layoutManager.textLineForOffset(offset) else {
            return NSRange(location: offset, length: 0)
        }

        let fragments = line.data.typesetter.lineFragments
        let lineFragment = if offset == line.range.max {
            fragments.last
        } else {
            fragments.getLine(atOffset: offset - line.range.location)
        }
        guard let lineFragment else {
            return NSRange(location: offset, length: 0)
        }

        let lineText = string.substring(with: line.range)
        let lineEndingLength = LineEnding(line: lineText)?.length ?? 0
        let lineBound = delta > 0
        ? line.range.location + min(
            lineFragment.range.max,
            line.range.length - lineEndingLength
        )
