...
public class TextSelectionManager: NSObject {
    ...
    /// Updates the visible selection without notifying editor peripherals during a live mouse drag.
    func updateSelectedRangeDuringMouseDrag(_ range: NSRange) -> Bool {
        if textSelections.count == 1 {
            let selection = textSelections[0]
            guard selection.range != range else { return false }
    ...
    /// Publishes the final selection after a live mouse drag finishes.
    func finishMouseDragSelection(notifyObservers: Bool) {
        updateSelectionViews()
