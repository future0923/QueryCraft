...
class Highlighter: NSObject {
    ...
    private var visibleLayoutRefreshTask: Task<Void, Never>?
    ...
    deinit {
        visibleLayoutRefreshTask?.cancel()
        self.attributeProvider = nil
        self.textView = nil
        self.highlightProviders = []
    }

    private func scheduleVisibleLayoutRefresh() {
        visibleLayoutRefreshTask?.cancel()
        visibleLayoutRefreshTask = Task { @MainActor [weak self] in
            await Task.yield()
            guard !Task.isCancelled, let self, let textView else { return }
            textView.layoutManager.layoutLines(in: textView.visibleRect)
            textView.needsDisplay = true
            visibleLayoutRefreshTask = nil
        }
    }
}
