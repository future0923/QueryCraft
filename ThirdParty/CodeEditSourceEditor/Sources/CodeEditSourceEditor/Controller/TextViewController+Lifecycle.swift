...
extension TextViewController {
    ...
    override public func viewDidDisappear() {
        ...
        if SuggestionController.shared.model.activeTextView === self {
    ...
    func handleCommand(event: NSEvent, modifierFlags: NSEvent.ModifierFlags) -> NSEvent? {
        ...
        if isReturn,
           modifierFlags == [commandKey, .shift],
           routeKeyCommand(.commandShiftReturn)
        {
            return nil
        }
        if isReturn,
           modifierFlags == commandKey,
           routeKeyCommand(.commandReturn)
        {
            return nil
        }
        if modifierFlags == commandKey,
           event.charactersIgnoringModifiers?.lowercased() == "i",
           routeKeyCommand(.commandI)
        {
            return nil
        }
        if modifierFlags == commandKey,
           event.charactersIgnoringModifiers?.lowercased() == "s",
           routeKeyCommand(.commandS)
        {
            return nil
        }
        if modifierFlags == controlKey,
           event.charactersIgnoringModifiers?.lowercased() == "c",
           routeKeyCommand(.commitTransaction)
        {
            return nil
        }
        if modifierFlags == controlKey,
           event.charactersIgnoringModifiers?.lowercased() == "r",
           routeKeyCommand(.rollbackTransaction)
        {
            return nil
        }
    ...
    private func routeKeyCommand(_ command: TextViewKeyCommand) -> Bool {
        for coordinator in textCoordinators.values() {
            if let handler = coordinator as? any TextViewKeyCommandHandling,
               handler.handleTextViewKeyCommand(
                    command,
                    textView: textView
               )
            {
