...
struct CompletionSessionLayout {
...
extension SuggestionController {
    func setCompletionAnchor(cursorRect: NSRect, font: NSFont) {
        completionSessionLayout = CompletionSessionLayout(
    ...
    func clearCompletionAnchor() {
        completionSessionLayout = nil
    }

    func lockedCompletionSize(for proposedSize: NSSize) -> NSSize {
        guard popover == nil, completionSessionLayout != nil else {
            return proposedSize
        }
        if let size = completionSessionLayout?.size {
            let expandedSize = NSSize(
                width: max(size.width, proposedSize.width),
                height: max(size.height, proposedSize.height)
            )
            completionSessionLayout?.size = expandedSize
            return expandedSize
        }
        completionSessionLayout?.size = proposedSize
        return proposedSize
    }

    func repositionWindowAtCompletionAnchor() {
        guard let completionSessionLayout else { return }
        constrainWindowToScreenEdges(
            cursorRect: completionSessionLayout.cursorRect,
            font: completionSessionLayout.anchorFont
        )
    }
    ...
    func updateWindowSize(newSize: NSSize) {
        ...
        let sessionSize = lockedCompletionSize(for: newSize)

        window.contentMaxSize = sessionSize
        window.contentMinSize = sessionSize

        window.setContentSize(sessionSize)
        repositionWindowAtCompletionAnchor()
    }
