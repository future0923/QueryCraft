...
extension SourceEditor {
    ...
    public class Coordinator: NSObject {
        ...
        private var lastSynchronizedScrollPosition: CGPoint?
        ...
        func synchronizeScrollPosition(_ position: CGPoint?, controller: TextViewController) {
            // An unchanged binding can lag behind a native scroll until the next runloop. It is not a request
            // to restore that old viewport, even if cursor updates have already triggered another SwiftUI render.
            guard position != lastSynchronizedScrollPosition else { return }
            lastSynchronizedScrollPosition = position
            guard let position, let scrollView = controller.scrollView,
                  position != scrollView.contentView.bounds.origin else { return }
            scrollView.scroll(scrollView.contentView, to: position)
        ...
        func textControllerScrollDidChange(_ notification: Notification) {
            guard !isUpdatingFromRepresentable,
                  editorState.scrollPosition == lastSynchronizedScrollPosition,
                  let controller = notification.object as? TextViewController else {
                return
            }
            let currentPosition = controller.scrollView.contentView.bounds.origin
            if editorState.scrollPosition != currentPosition {
                lastSynchronizedScrollPosition = currentPosition
