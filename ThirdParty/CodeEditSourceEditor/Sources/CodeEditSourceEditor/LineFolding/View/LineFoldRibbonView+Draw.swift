...
extension LineFoldRibbonView {
    ...
    private func drawCollapsedFold(
        ...
        context.setStrokeColor(resolvedCGColor(foldedIndicatorChevronColor))
        context.setLineCap(.round)
        context.setLineJoin(.round)
        context.setLineWidth(1.3)

        context.setFillColor(resolvedCGColor(foldedIndicatorColor))
    ...
    private func drawHoveredFold(
        ...
        context.setFillColor(
            resolvedCGColor(
                hoverFillColor.withAlphaComponent(hoveringFold.progress)
            )
        )
        context.setStrokeColor(
            resolvedCGColor(
                hoverBorderColor.withAlphaComponent(hoveringFold.progress)
            )
        )
    ...
    private func drawNestedFold(
        ...
        context.setFillColor(resolvedCGColor(markerColor))
        context.addPath(roundedRect.cgPathFallback)
    ...
    private func drawOutline(
        ...
        context.setFillColor(resolvedCGColor(markerBorderColor))
    ...
    private func resolvedCGColor(_ color: NSColor) -> CGColor {
        color.resolved(for: effectiveAppearance).cgColor
    }
}
