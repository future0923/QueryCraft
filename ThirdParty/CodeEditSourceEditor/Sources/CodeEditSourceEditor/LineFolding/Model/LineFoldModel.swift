...
class LineFoldModel: NSObject, NSTextStorageDelegate, ObservableObject {
    ...
    init(controller: TextViewController, foldView: NSView) {
        ...
        (textChangedStream, textChangedStreamContinuation) = AsyncStream<Void>.makeStream(bufferingPolicy: .bufferingNewest(1))
        ...
        cacheListenTask = Task { @MainActor [weak self, weak foldView] in
            for await result in await calculator.valueStream {
                guard !Task.isCancelled, let self else { break }
                guard result.revision == self.revision else { continue }
                self.foldCache = result.storage
    ...
    /// Replacing NSTextStorage is not a character-edit notification. Invalidate
    /// both the old iterator and its cache explicitly before the next read.
