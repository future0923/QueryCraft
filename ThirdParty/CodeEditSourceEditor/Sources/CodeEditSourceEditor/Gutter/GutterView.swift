...
public class GutterView: NSView {
    ...
    private var minimumDigitCount = 3
    ...
    func configureLineNumberSpacing(leadingPadding: CGFloat, minimumDigitCount: Int) {
        let minimumDigitCount = max(1, minimumDigitCount)
        let insets = EdgeInsets(leading: max(0, leadingPadding), trailing: edgeInsets.trailing)
        guard edgeInsets != insets || self.minimumDigitCount != minimumDigitCount else { return }
        edgeInsets = insets
    ...
    func updateWidthIfNeeded() {
        ...
        let lineStorageDigits = max(minimumDigitCount, String(textView.layoutManager.lineCount).count)
