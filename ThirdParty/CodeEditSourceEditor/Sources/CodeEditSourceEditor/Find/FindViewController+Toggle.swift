...
extension FindViewController {
    ...
    func showFindPanel(animated: Bool = true) {
        ...
        // Let the hosting view finish its current SwiftUI render before forcing
        // AppKit layout. Laying it out synchronously here causes a reentrant
        // NSHostingView layout pass when the panel becomes visible.
        DispatchQueue.main.async { [weak self] in
            guard let self, self.viewModel.isShowingFindPanel else { return }
            self.view.layoutSubtreeIfNeeded()

            self.conditionalAnimated(animated) {
                // SwiftUI refuses to return the correct `findPanel.fittingSize`,
                // so the panel height remains an explicit value.
                self.viewModel.target?.findPanelWillShow(
                    panelHeight: self.viewModel.panelHeight
                )
